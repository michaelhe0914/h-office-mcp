...
// ─── Tool: check_login ─────────────────────────────────────────────
server.tool("check_login", "檢查 h-office 系統的登入狀態。", {}, async () => {
...
server.tool("list_product_classes", "列出出書統計表可用的產品類別。需要先登入。", {}, async () => {
    ...
    try {
        ...
        return {
            content: [
                {
                    type: "text",
                    text: `📁 可用的產品類別 (共 ${classes.length} 個)：\n\n` +
...
// ─── Tool: query_sales ─────────────────────────────────────────────
server.tool("query_sales", "查詢金安出版社的出書統計表。支援按日期區間和產品類別篩選。日期格式為 YYYY-M-D（例如 2025-3-25）。", {
    begin_date: z
        .string()
...
server.tool("query_sales_raw", "查詢出書統計表並回傳原始 HTML（用於偵錯或分析頁面結構）。", {
    ...
    try {
        ...
        const contentMatch = res.body.match(/<div id='content'>([\s\S]*?)<\/div>\s*<\/div>\s*<\/body>/);
