...
// for 中區 in the date range 114.4.26~114.9.25 (= 2025/4/26 ~ 2025/9/25)
// Collect all records for the 3 sales reps: 蔡榮訓、曹原菘、林廣強
...
async function run() {
  ...
  for (const pc of singleVolumeClasses) {
    ...
    // Filter for 中區 and target sales reps
    const targetRecords = result.records.filter(r => 
      r.zone === "中區" && targetSales.includes(r.sales)
    );
    ...
    if (targetRecords.length > 0) {
      ...
      console.log(`  No matching records for 中區 target sales reps.`);
    }
  }
  
  // Save all records to JSON for further processing
  fs.writeFileSync('query_results_central_single_volume.json', JSON.stringify(allRecords, null, 2), 'utf-8');
  console.log(`\n=== Total records collected: ${allRecords.length} ===`);
  console.log(`Saved to query_results_central_single_volume.json`);
